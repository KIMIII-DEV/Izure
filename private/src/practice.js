...
/* Die Themen tragen einen Markdown-artigen Fließtext (`c`), Kernpunkte
   (`k`) und die Klausurrelevanz (`r`). Die Lieferung zeigte nur die
   Kurzbeschreibung — damit wären 108 Themen auf je eine Zeile
   zusammengeschrumpft und der eigentliche Stoff unsichtbar geblieben.

   Statt eine Markdown-Bibliothek einzubinden, wird hier zuerst ALLES
   escaped und erst danach das erlaubte Wenige zu HTML gemacht: **fett**,
   Aufzählungen, Absätze. Fremdes Markup kann so nicht durchrutschen. */
function md(src){
